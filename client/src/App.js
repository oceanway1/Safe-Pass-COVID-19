...
import getGlobalCount from "./components/helper/getAllDashBoard"
...
function App() {
  ...
  const [death, setDeathd] = useState(0);
  const [recovered, setRecoveredd] = useState(0);
  const [confirmed, setconfirmedd] = useState(0)

  useEffect(() => {
    getGlobalCount(setDeathd, setconfirmedd, setRecoveredd)
  }, [])
          ...
          <Switch>
            ...
            <Route path="/news">
              <Grid container spacing={3}>
                <Grid item xs={3} style={{ boder: '0px solid red' }}>
                  <Card confirmedd={confirmed}></Card>
                  <Card2 death={death}></Card2>
                  <Card3 recovered={recovered}></Card3>
                </Grid>
                <Grid container item xs={9} text-align="right" padding-top="50px">
                  <MapGL
                    {...viewport}
